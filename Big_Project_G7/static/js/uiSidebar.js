document.addEventListener("DOMContentLoaded", function() {
    ...
    const boothUrl = `${window.location.origin}/get_booth_info/`;  // Replace with your actual URL
    fetch(boothUrl)
        .then(response => response.json())
        .then(data => {
            const boothlist = JSON.parse(data);
            const sidebarShowBookmarksButton = document.getElementsByClassName('bookMlist');
            if (sidebarShowBookmarksButton.length > 0) {
                Array.from(sidebarShowBookmarksButton).forEach(function(button) {
